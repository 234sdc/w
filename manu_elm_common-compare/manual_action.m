function [reward_temp,cost_temp,cost_store,next,action,spare_queue]=manual_action(input,ini_health,spare_queue,num,states_vector,PHM_noise)
%     [~,action]=max(act);


    t=input(1);
    health=input(2);
    health_sense=health+health*PHM_noise*randn;
    spare_store=input(3);
    length=input(4);
