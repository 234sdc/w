function [reward_temp,cost_temp,next,spare_queue]=reward_cal(input,act,ini_health,spare_queue,num)
    [~,action]=max(act);
    order_t=10;
    store_cost_rate=0.5;
    t=input(1);
    ...
    switch action
        case 1 % take the mission and order
            ...
            spare_queue=[spare_queue t+order_t];% put the new order spare part into spare queue
                        store_days=mission_timepoint-spare_queue;
    ...
    %% next state
        ...
        input=input';   
        z1=mission_profile(1,extre_mission);


        input(1:end-1,:)=input(2:end,:);% remove the state data of current time
        input(end,:)=z1;% add the newest state data at the end
